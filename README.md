...
## Integration into Multires-ML-Microscopy

The 3D microglia segmentation and classification pipeline has also been integrated into the Multires-ML-Microscopy application.

The integrated implementation is available on the `Microglia-3D` branch:

https://github.com/USonixGroup/Multires-ML-Microscopy/tree/Microglia-3D

This branch incorporates the 3D microglia workflow into the existing MATLAB graphical user interface and includes the runtime segmentation and classification functions, trained classifier, and demonstration image.
